...
const val USAGE = """Основные команды:
    ...
    !skip [count] — Пропускает следующие count треков (включая текущий), по умолчанию count=1.
    !next[count] [track name / track url / playlist url] — Ставит указанный трек следующим (в начало очереди).
    !force[count] [track name / track url / playlist url] — Ставит указанный трек скипая текущий (!next + !skip).
    !queue — Выводит текущую очередь треков.
    !current — Выводит название текущего трека и прогресс проигрывания.
    !status — !current + !queue.
    !link — Выводит ссылку на текущий трек (при возможности).

    !mute - Бот больше не пингует вас на каждое сообщение.
    !pause - Ставит текущий трек на паузу.
    !resume - Снимает текущий трек с паузы.
    !repeat [off] — Включает/выключает режим повторения одного трека.
    !stay [off] - Включает/выключает режим "остаться в войсе после конца треков".
    !join - Джоинится в войс и не выходит (ставит `!stay on`), полезно для TTS.
    !echo [off] - Джоинится в войс и озвучивает все последущие сообщения в чате, оставаясь в нем.
...
Приколы:
    ...
    !long [count] [-n] - Неизданный куплет Славы КПСС.
